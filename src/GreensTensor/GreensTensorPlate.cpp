...
GreensTensorPlate::GreensTensorPlate(const std::string &input_file)
  ...
  reflection_coefficients = ReflectionCoefficientsFactory::create(input_file);
  ...
  // read parameters
  za = root.get<double>("GreensTensor.za");
  delta_cut = root.get<double>("GreensTensor.delta_cut");
  rel_err(0) = root.get<double>("GreensTensor.rel_err_0");
  rel_err(1) = root.get<double>("GreensTensor.rel_err_1");
...
void GreensTensorPlate::calculate_tensor(double omega, vec::fixed<2> k,
  ...
  const double kx = k(0);
  const double ky = k(1);
  const double k_quad = kx * kx + ky * ky;
  ...
  const double omega_quad = omega_abs * omega_abs;
  ...
  const std::complex<double> pre = (2 * M_PI) * exp(-(2 * za) * kappa);
  const std::complex<double> prefactor_s =
      pre * r_s * omega_quad / (k_quad - omega_quad);
  const std::complex<double> prefactor_p = pre * r_p;
...
void GreensTensorPlate::integrate_k(double omega, cx_mat::fixed<3, 3> &GT,
  ...
  auto F_xx = [=](double x) -> double {
    return integrand_1d_k(x, omega, {0, 0}, fancy_complex, weight_function);
  };
  GT(0, 0) = cquad(F_xx, 0, M_PI, rel_err(1), 0) / M_PI;

  // the yy element
  auto F_yy = [=](double x) -> double {
    return integrand_1d_k(x, omega, {1, 1}, fancy_complex, weight_function);
  };
  GT(1, 1) = cquad(F_yy, 0, M_PI, rel_err(1), 0) / M_PI;

  // the zz element
  auto F_zz = [=](double x) -> double {
    return integrand_1d_k(x, omega, {2, 2}, fancy_complex, weight_function);
  };
  GT(2, 2) = cquad(F_zz, 0, M_PI, rel_err(1), 0) / M_PI;

  // the zx element
  auto F_zx = [=](double x) -> double {
    return integrand_1d_k(x, omega, {2, 0}, fancy_complex, weight_function);
  };
...
double GreensTensorPlate::integrand_1d_k(double phi, double omega,
  ...
  const double kappa_cut = delta_cut / (2 * za);
  ...
  auto F = [=](double x) -> double {
    return integrand_2d_k(x, omega, phi, indices, fancy_complex,
                          weight_function);
  };

  // Calculate low-temperature edge
  const double edge = std::abs(omega / (v * cos_phi));
  ...
  if ((kappa_cut > edge) && (2 * za / v < beta)) {
    ...
    result +=
        cquad(F, edge, kappa_cut, rel_err(0), std::abs(result) * rel_err(0));
  } else {
    result = cquad(F, 0, kappa_cut, rel_err(0), 0);
  }
  result +=
      cquad(F, -std::abs(omega), 0, rel_err(0), std::abs(result) * rel_err(0));
...
double GreensTensorPlate::integrand_2d_k(double kappa_double, double omega,
  ...
  const double v_quad = v * v;
  const double omega_quad = omega * omega;
  const double cos_phi = cos(phi);
  const double cos_phi_quad = cos_phi * cos_phi;
  const double sin_phi_quad = 1.0 - cos_phi_quad;
  ...
  const double k =
      (sqrt((kappa_quad + omega_quad) - kappa_quad * v_quad * cos_phi_quad) +
       v * omega * cos_phi) /
      (1.E0 - v_quad * cos_phi_quad);
  const double k_quad = k * k;

  // Define the Doppler-shifted frequency
  const double omega_pl = (omega + k * cos_phi * v);
  ...
  const double omega_pl_abs = std::abs(omega_pl);
  ...
  const std::complex<double> prefactor = std::abs(kappa_complex) *
  ...
  const std::complex<double> prefactor_p = prefactor * r_p * kappa_complex;
