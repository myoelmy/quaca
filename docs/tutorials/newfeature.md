...
All permittivities are children of this class, so they inherit from it and they have to implement the two functions `calculate(double omega)` and `calculate_times_omega (double omega)`.
...
public:
  // constructors
  PermittivityLorentzOhmic(double eps_inf, double alpha_0, double omega_0,
                           double gamma);
  explicit PermittivityLorentzOhmic(const std::string &input_file);
...
PermittivityLorentzOhmic::PermittivityLorentzOhmic(
  ...
  alpha_0 = root.get<double>("Permittivity.alpha_0");
...
std::complex<double> PermittivityLorentzOhmic::calculate_times_omega(double omega) const {
  ...
  // calculate the result
  result =
      eps_inf * omega - alpha_0 * omega_0 * omega_0 /
                    (omega_0 * omega_0 / omega - omega - I * gamma);
